...
const Service = (function()
{
	...
	me.incDay = function(mapInstanceId, getMosquitoes, cbSuccess, cbFail)
	{
		setTimeout(function(){
			...
			if (wet) {
				...
				kpis.infectiousMosquitoes = Math.floor(kpis.infectiousMosquitoes * (1 + Math.random() / 40));
			} else {
				kpis.mosquitoes = Math.floor(kpis.mosquitoes * (1 - Math.random() / 60));
				kpis.infectiousMosquitoes = Math.floor(kpis.infectiousMosquitoes * (1 - Math.random() / 60));
			}
			kpis.mosquitoes = Math.round(kpis.mosquitoes * mosFactor);
			kpis.infectiousMosquitoes = Math.round(kpis.infectiousMosquitoes * mosFactor);
			...
			if (illRandom<chanceOfLessIll && kpis.infectedHuman) {// 5% of 1 less infectedHuman
				kpis.infectedHuman--;
			}
			_.forEach(kpis.habitatsLifeInfo, (p) => {
			...
			sprayPuddlesLeft--;
			let dialog = null;
			if (kpis.infectedHuman > 22 && !alreadyFailed) {
				alreadyFailed = true;
				dialog = {
					type: DIALOG_TYPE.GAME_OVER,
					title: 'Game Over',
					text: `Level failed. There are ${kpis.infectedHuman} ill people in the village, this is too much...`
				};
	...
	const _data = {
		...
		time: {
			...
			periods:[
				{
					climate: PERIOD_TYPE.WET,
					duration: 90 // duration in days
				},
				{
					climate: PERIOD_TYPE.DRY,
					duration: 90 // duration in days
				},
				{
					climate: PERIOD_TYPE.WET,
